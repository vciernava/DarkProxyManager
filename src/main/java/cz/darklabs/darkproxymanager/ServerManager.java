...
public class ServerManager {
    ...
    private static final int startingPort = 30000;
    private static final int endingPort = 31000;
    ...
    public static void createLobbyServer(DarkProxyManager DarkProxyManager) throws IOException, ApiException {
        ...
        V1Container container = new V1Container().name("lobby-server-container").image(Configurations.getConfig().getLobbyImage()).env(envVars).imagePullPolicy("Always").ports(Collections.singletonList(new V1ContainerPort().containerPort(port).protocol("TCP")));
    ...
    public static void deleteLobbyServer(UUID uuid, DarkProxyManager DarkProxyManager) throws IOException, ApiException {
        ...
        PendingLobbyServers.getLobbies().remove(uuid);
    }

    public static V1PodList getPods() throws IOException {
        ApiClient client = Config.defaultClient();
        Configuration.setDefaultApiClient(client);
        CoreV1Api api = new CoreV1Api();
